...
- If a code block or code example is included in the evidence, the persistent code rule must be stated in natural
  language outside it.
- Use the filename and file location only to identify the governed subject, intended actor, purpose, and scope.
- A title, section heading, introduction, or immediately surrounding sentence and its directly governed body, list, or
  table may be evaluated as one contiguous passage. Include every contextual part used in quote.
- Do not infer scope, a requirement, prohibition, recommendation, permission, persistence, or a concrete specification
  that is not stated anywhere in the combined passage.
...
- A condition is persistent when it applies generally whenever a code element of the same kind is created, added, or
  modified and defines content or structure that remains after work is complete, or when existing code must continue
  to satisfy it. It need not contain wording equivalent to "in the future" or "every time."
- Whether the condition is satisfied can be determined from the content or structure of source code or test code after
  modification.
- A condition is not persistent if it ends when one named change, repair, replacement, addition, deletion,
  refactoring, migration, port, adaptation, fix, troubleshooting task, or problem-solving task is complete.
...
- A name, prefix, suffix, identifier, naming pattern, or fixed value that one of those names or code identifiers must
  use.
- A complete declaration, import statement, annotation, or comment form that maintained source code or test code must
  contain, omit, or place at a specified position or in a specified order.
- A required number, composition, kind, or placement of automated tests for each change or code element of a specified
  kind.
- A language, runtime, platform, API, version, or previously released interface with which source code, test code, or
  a named maintained package or API must remain compatible. A numeric version is not required when the rule explicitly
  requires compatibility with a previously released interface.
...
- Language-general syntax, declaration form, keyword, character set, casing choice, naming format, or coding practice
  that is neither tied to a name or code identifier defined by the software or to a relative source-code or test-code
  path, nor specified as a complete form that maintained code must contain, omit, or place at a specified position or
