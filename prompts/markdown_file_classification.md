...
- Source code or test code for a plugin, workflow, policy, backend, application, or custom extension: content explicitly
  states that the code itself is maintained as part of the software. Placement at a repository-relative path or in a
  source package, or an instruction to import or register the code, does not by itself establish this condition.
...
- A statement governing a runtime value, state, input, output, behavior, process, activity, or runtime configuration
  rather than source code or test code. This includes whether a value or combination of values supplied by a software
  user through configuration is supported.
...
- A statement specifying a condition or method for a software user to install, run, select, enable, make available, or
  use software or one of its features. This includes a condition governing code or configuration that the user creates
  or modifies for that purpose, regardless of location.
...
- A statement governing any of the following for a formatter, linter, build tool, code generator, or other development
  tool is out of scope even if the governed configuration, file, or entry has a source-code filename extension, is
  located at a repository-relative path, or causes source code, test code, or configuration to be generated or modified:
    - A configuration option, configuration value, or combination of them that the tool accepts as input.
    - A file or entry whose stated purpose is to configure the tool, register inputs for the tool, or drive the tool.
...
- Prose that only does one of the following is descriptive and does not establish explicit normativity:
    - A sentence that states as fact a completed implementation or the current or past content, structure, manner of
      writing, placement, storage location, naming, organization, dependency relationship, supported environment, use,
      behavior, presence, absence, or routing of calls in code, including the availability or importability of an API,
      helper, class, method, object, or similar facility.
    - A sentence that describes a planned implementation, planned change, or design proposal.
    - A passage that explains or defines a concept, structure, purpose, design, design decision, code state, or pattern
      for readers to understand or distinguish. This includes a component's design intent, an explanation of why the
      implementation, state, or call routing described by the passage was selected or necessary, and a condition
      presented only as a constraint that determined a completed implementation. A statement that the explained state
      has no exceptions, is limited to particular subjects, or lacks a particular call does not by itself state a
      requirement or prohibition. This includes a passage that assigns a name to a code state or pattern and defines its
      content.
...
- A specified language, runtime, platform, API, or version with which source code, test code, or changes to either must
  remain compatible. Using such an item only to select syntax, a declaration form, an API call, or a programming
  technique is not a compatibility condition.
